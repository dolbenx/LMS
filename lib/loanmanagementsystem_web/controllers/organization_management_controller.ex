...
use PipeTo.Override
  ...
  def sms_maintainance(conn, _params) do

    smses = Loanmanagementsystem.Notifications.Sms|> Repo.all()
    render(conn,"sms_maintainance.html", smses: smses)
  end
  ...
  def admin_message_resend(conn, params) do
    msg = Loanmanagementsystem.Notifications.get_sms!(params["id"])

    Ecto.Multi.new()
    |> Ecto.Multi.update(
      :message_resend,
      Sms.changeset(msg, Map.merge(params, %{"status" => "READY"}))
    )
    |> Ecto.Multi.run(:user_logs, fn _,
                                     %{message_resend: _message_resend} ->
      UserLogs.changeset(%UserLogs{}, %{
        activity: "message Resend Successfully Done",
        user_id: conn.assigns.user.id
      })
      |> Repo.insert()
    end)
    |> Repo.transaction()
    |> case do
      {:ok, %{message_resend: _message_resend}} ->
        json(conn, %{
          data: "Message successfully Resent"
        })
