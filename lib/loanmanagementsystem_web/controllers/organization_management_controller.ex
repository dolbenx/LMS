defmodule LoanmanagementsystemWeb.OrganizationManagementController do
  ...
  alias Loanmanagementsystem.Maintenance.{Currency, Alert_Maintenance, Holiday_mgt, Qfin_Brance_maintenance}
  ...
  plug LoanmanagementsystemWeb.Plugs.Authenticate,
      [module_callback: &LoanmanagementsystemWeb.OrganizationManagementController.authorize_role/1]
      when action not in [
          :action,
...
use PipeTo.Override
  ...
  def admin_create_currency(conn, params) do
    ...
    get_isocode = Repo.get_by(Currency, isoCode: isocode)
    get_currency_name = Repo.get_by(Currency, name: currency_name)

    case is_nil(get_currency_name) do
      true ->
        case is_nil(get_isocode) do
          true ->
          Ecto.Multi.new()
          ...
          _ ->
          conn
          |> put_flash(:error, "ISOCODE #{isocode} already exists")
          |> redirect(to: Routes.organization_management_path(conn, :currency_maintianance))
        end

        _ ->
        conn
        |> put_flash(:error, "Currency Name #{currency_name} already exists")
        |> redirect(to: Routes.organization_management_path(conn, :currency_maintianance))
    end
  end
  ...
  def view_qfin_add_employee(conn, _params) do
    employees = Loanmanagementsystem.OperationsServices.get_qfin_employees()
  ...
  def activate_holiday(conn, params) do

    holiday = Loanmanagementsystem.Maintenance.get_holiday_mgt!(params["id"])

    Ecto.Multi.new()
    |> Ecto.Multi.update(
      :activate_holiday,
      Holiday_mgt.changeset(holiday, Map.merge(params, %{"status" => "APPROVED"}))
    )
    |> Ecto.Multi.run(:user_logs, fn _, %{activate_holiday: activate_holiday} ->
      UserLogs.changeset(%UserLogs{}, %{
        activity: "Activated #{activate_holiday.holiday_description} Successfully ",
        user_id: conn.assigns.user.id
      })
      |> Repo.insert()
    end)
    |> Repo.transaction()
    |> case do
      {:ok, %{activate_holiday: activate_holiday}} ->
        json(conn, %{data: "#{activate_holiday.holiday_description} activated successfully"})
  ...
  def deactivate_holiday(conn, params) do
    ...
    |> Ecto.Multi.update(
      :deactivate_holiday,
      Holiday_mgt.changeset(holiday, Map.merge(params, %{"status" => "DISABLED"}))
    )
    |> Ecto.Multi.run(:user_logs, fn _, %{deactivate_holiday: deactivate_holiday} ->
      UserLogs.changeset(%UserLogs{}, %{
        activity: "Disabled #{deactivate_holiday.holiday_description} Successfully ",
        user_id: conn.assigns.user.id
      })
      |> Repo.insert()
    end)
    |> Repo.transaction()
    |> case do
      {:ok, %{deactivate_holiday: deactivate_holiday}} ->
        json(conn, %{data: "#{deactivate_holiday.holiday_description} disabled successfully"})
  ...
  def traverse_errors(errors),
    ...
    def authorize_role(conn) do
      case Phoenix.Controller.action_name(conn) do
        act when act in ~w(new create)a -> {:organization_mgt, :create}
        act when act in ~w(index view)a -> {:organization_mgt, :view}
        act when act in ~w(update edit)a -> {:organization_mgt, :edit}
        act when act in ~w(change_status)a -> {:organization_mgt, :change_status}
        _ -> {:organization_mgt, :unknown}
      end
    end
