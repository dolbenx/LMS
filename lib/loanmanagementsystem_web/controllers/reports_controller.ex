defmodule LoanmanagementsystemWeb.ReportsController do
  ...
  plug LoanmanagementsystemWeb.Plugs.Authenticate,
       ...
       when action not in [
        ...
        :all_individuals_employees,
        :all_loans_item_lookup,
        :admin_view_all_customer,
        :all_customers_item_lookup,
        :disbursements,
        :disbursed_loans_item_lookup,
        :outstanding_loan_balance,
        :repayment_listing,
        :outstanding_loan_balance_item_lookup,
        :repayment_listing_item_lookup,
        :expected_repayment_report,
        :last_installmemet_overpayment,
        :last_installmemet_overpayment_lookup,
        :repayment_from_writeoff,
        :repayment_from_writeoff_lookup,
        :written_off_loan_report,
        :written_off_loan_report_lookup,
        :arrears_report,
        :arrears_loan_report_lookup,
        :cashflow_statement,
        :cashflow_statement_report_lookup,
        :financial_position,
        :financial_position_report_lookup,
        :expected_repayments_item_lookup,
        :daily_disbursements,
        :daily_disbursements_lookup,
        :daily_deliquency,
        :daily_deliquency_lookup,
        :admin_user_logs
  ...
  def all_loans(conn, _params),
    do:
      render(conn, "all_loans.html", products: Loanmanagementsystem.Products.list_tbl_products(), loans: Loanmanagementsystem.Operations.get_all_loans())

  def expected_repayment_report(conn, _params), do: render(conn, "expected_repayment_report.html")
  def outstanding_loan_balance(conn, _params) do
    relationship_officer = Loan.list_customer_relationship_officer()
    render(conn, "outstanding_loan_balance.html", relationship_officer: relationship_officer)
  end
  def repayment_listing(conn, _params), do: render(conn, "repayment_listing.html")
  def last_installmemet_overpayment(conn, _params), do: render(conn, "last_installmemet_overpayment.html")
  def disbursements(conn, _params), do: render(conn, "disbursements.html")
  def repayment_from_writeoff(conn, _params), do: render(conn, "repayment_from_writeoff.html")
  def written_off_loan_report(conn, _params), do: render(conn, "writtenoff_loan_report.html")
  def arrears_report(conn, _params) do
    relationship_officer = Loan.list_customer_relationship_officer()
    render(conn, "arrears_report.html", relationship_officer: relationship_officer)
  end
  def daily_disbursements(conn, _params), do: render(conn, "daily_disbursment_conso.html")
  def daily_deliquency(conn, _params), do: render(conn, "daily_deliquency_per_agent.html")

  def cashflow_statement(conn, params) do
    cashflow =  Loanmanagementsystem.FinancialReports.cashflow_details_report(params["start_date"], params["end_date"])
   render(conn, "cashflow_report.html", cashflow: cashflow)
  end

  def financial_position(conn, params) do
    financial_postion = Loanmanagementsystem.FinancialReports.financial_position_report(params["frequency"], params["date"])
    render(conn, "financial_position.html", financial_postion: financial_postion)
  end
    ...
    def all_loans_item_lookup(conn, params) do
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.Operations.loan_list(search_params, start, length)
      IO.inspect(results, label: "results results results results results results")
    ...
    def daily_disbursements_lookup(conn, params) do
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.Operations.daily_disbursed_loan_list(search_params, start, length)
      total_entries = total_entries(results)
      entries = entries(results)
    ...
    def daily_deliquency_lookup(conn, params) do
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.Operations.daily_deliquency_list(search_params, start, length)
      total_entries = total_entries(results)
      entries = entries(results)
    ...
    def disbursed_loans_item_lookup(conn, params) do
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.Operations.disbursed_loan_list(search_params, start, length)
      total_entries = total_entries(results)
      entries = entries(results)
    ...
    def outstanding_loan_balance_item_lookup(conn, params) do
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.Operations.outstanding_loan_balance(search_params, start, length)
      total_entries = total_entries(results)
      entries = entries(results)
    ...
    def repayment_listing_item_lookup(conn, params) do
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.Operations.repayment_loan_list(search_params, start, length)
      total_entries = total_entries(results)
      entries = entries(results)
    ...
    def expected_repayments_item_lookup(conn, params) do
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.Operations.expected_repayment_loan_list(search_params, start, length)
      total_entries = total_entries(results)
      entries = entries(results)
    ...
    def last_installmemet_overpayment_lookup(conn, params) do
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.Operations.last_repayment_overpay_list(search_params, start, length)
    ...
    def repayment_from_writeoff_lookup(conn, params) do
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.Operations.last_repayment_writeoff_list(search_params, start, length)
    ...
    def written_off_loan_report_lookup(conn, params) do
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.Operations.written_off_loan_report_list(search_params, start, length)
    ...
    def arrears_loan_report_lookup(conn, params) do
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.Operations.arrears_report_list(search_params, start, length)
    ...
    def cashflow_statement_report_lookup(conn, params) do
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.FinancialReports.cashflow_report(search_params, start, length)
    ...
    def financial_position_report_lookup(conn, params) do
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.FinancialReports.cashflow_report(search_params, start, length)
    ...
    def all_customers_item_lookup(conn, params) do
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.Operations.list_customer(search_params, start, length)
  ...
  def admin_view_all_customer(conn, _params) do
    render(conn, "admin_view_all_customer.html")
  end
  ...
  def compose_report_select(query) do
    ...
    |> select([a], %{
      account_no: fragment(
        "select distinct case when exists(select ac_gl_no from  tbl_chart_of_accounts where ac_gl_no = ? ) then account_no else '7100'  end from tbl_trans_log where  account_no = ? and  process_status = 'APPROVED' and batch_status = 'APPROVED'   group by drcr_ind, account_no",
        a.account_no,
        a.account_no
      ),
      ...
      account_name:
      fragment(
        "select distinct case when exists(select ac_gl_no from  tbl_chart_of_accounts where ac_gl_no = ? ) then account_name else '(Receivables Control)'  end from tbl_trans_log where  account_no = ? and  process_status = 'APPROVED' and batch_status = 'APPROVED'   group by drcr_ind, account_no, account_name",
        a.account_no,
        a.account_no
      ),
      drcr_ind: a.drcr_ind,
      dr_amount:
        fragment(
          "select case when  drcr_ind = ? then sum(lcy_amount + fcy_amount) else '0' end from tbl_trans_log where drcr_ind = 'D'  and account_no = ? and process_status = 'APPROVED' and batch_status = 'APPROVED' group by drcr_ind, account_no",
          a.drcr_ind,
          a.account_no
        ),
      cr_amount:
        fragment(
          "select case when  drcr_ind = ? then sum(lcy_amount + fcy_amount) else '0' end  from tbl_trans_log where drcr_ind = 'C' and account_no = ? and process_status = 'APPROVED' and batch_status = 'APPROVED' group by drcr_ind, account_no",
          a.drcr_ind,
          a.account_no
        )
    })
  ...
  def mini_statement_individuel_lookup(conn, params) do
    user_id = conn.assigns.user.id
    {draw, start, length, search_params} = search_options(params)
    results = Loanmanagementsystem.Operations.mini_statement_report(search_params, start, length, user_id)
    total_entries = total_entries(results)
    entries = entries(results)
  ...
  def historical_statement_individuel_lookup(conn, params) do
    user_id = conn.assigns.user.id
    {draw, start, length, search_params} = search_options(params)
    results = Loanmanagementsystem.Operations.historical_individual_statement_report(search_params, start, length, user_id)
    total_entries = total_entries(results)
    entries = entries(results)
  ...
  def all_individuals_employees(conn, _params) do
    branch = Loanmanagementsystem.Maintenance.list_tbl_branch()
    render(conn, "all_individuals_employees.html", branch: branch)
  end
