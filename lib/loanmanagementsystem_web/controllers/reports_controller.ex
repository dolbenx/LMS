defmodule LoanmanagementsystemWeb.ReportsController do
  ...
  plug LoanmanagementsystemWeb.Plugs.Authenticate,
       ...
       when action not in [
        ...
        :pipeline_loans_report,
        :loan_pipeline_report_item_lookup,
        :loans_report_by_funder,
        # :all_disbursed_loans_report_item_lookup,
        :end_of_day__totals_view,
        :compose_end_of_day_totals_list,
        :view_all_eod_entries,

        :loans_report_by_offtaker,
        :loan_per_offtaker_report_item_lookup,

        :all_disbursed_loans_report_item_lookup,
        :last_five_disbursed_loans_lookup_miz,
        :last_per_product_disbursed_lookup_miz,
        :all_innactive_clients_lookup,
        :active_loans_by_loan_officer,
        :due_collected_by_loan_officer,
        :get_over_due_loans_look_up,
        :corporate_loan_client_statement_report,
        :individual_loan_client_statement_report,
        :all_loans_by_funder_report,
        :all_funder_loan_report_lookup,
        :view_all_over_due_loans_by_funder,
        :all_funder_over_due_loan_report_lookup,
        :last_five_disbursed_loans_lookup_per_funder,
        :last_per_product_disbursed_lookup_per_funder
  ...
  def get_all_complete_balance_sheet(_search_params, page, size) do
    Journal_entries
  ...
  def get_all_complete_balance_sheet(_source, _search_params) do
    Journal_entries
    # |> balance_sheet_report_filter(search_params)
    # |> balance_sheet_date_filter(search_params)
    |> order_by(desc: :trans_date)
  ...
  def query_data_final(_search_params, page, size) do
    Journal_entries
    |> where([a], a.account_category != "Income")
    |> where([a], a.account_category != "Expenses")
  ...
  def query_data_final_income_statement(_search_params, page, size) do
    Journal_entries
  ...
  def query_data_final_trial_balance(_search_params, page, size) do
    Journal_entries
    |> where([a], a.process_status == "APPROVED")
    |> where([a], a.batch_status == "APPROVED")
    |> order_by([a], a.trn_dt)
  ...
  def query_data_final_general_ledger(_search_params, page, size) do
    Journal_entries
    |> where([a], a.process_status == "APPROVED")
    |> where([a], a.batch_status == "APPROVED")
    |> order_by([a], a.trn_dt)
  ...
  def get_all_complete_historic_loan_statement(_search_params, page, size) do
    Loans
  ...
  # CSV Report
  def get_all_complete_historic_loan_statement(_source, _search_params) do
    Loans
    |> join(:left, [a], u in "tbl_user_bio_data", on: a.id == u.userId)
    |> join(:left, [a, u], p in "tbl_products", on: a.product_id == p.id)
    |> where([a], a.loan_status == "ARCHIVED")
  ...
  def funder_last_five_disbursed_loans_lookup(conn, params) do
    {draw, start, length, search_params} = search_options(params)
    results = Loanmanagementsystem.Operations.last_five_disbursed_loans_per_funder(search_params, start, length)
    total_entries = total_entries(results)
    entries = entries(results)
  ...
  def last_per_product_disbursed_lookup_miz(conn, params) do
    {draw, start, length, search_params} = search_options(params)
    results = Loanmanagementsystem.Operations.get_active_loans_by_product_miz(search_params, start, length)
    total_entries = total_entries(results)
    entries = entries(results)
  ...
  def all_due_repayment_loans(conn, _params) do
    get_loan_funder = Loanmanagementsystem.Loan.get_loan_funder_details()
    products = Loanmanagementsystem.Products.list_tbl_products()
    offtaker_list = Loanmanagementsystem.Loan.get_offtaker_details()
    render(conn, "all_due_repayment_loans.html", offtaker_list: offtaker_list, products: products, get_loan_funder: get_loan_funder, loans: Loanmanagementsystem.Services.Services.admin_get_disbursed_loans() )
  end
  ...
  def all_overdue_repayment_loans(conn, _params) do
    ...
    products = Loanmanagementsystem.Products.list_tbl_products()
    offtaker_list = Loanmanagementsystem.Loan.get_offtaker_details()
    render(conn, "all_overdue_repayment_loans.html", offtaker_list: offtaker_list, products: products, get_loan_funder: get_loan_funder, loans: Loanmanagementsystem.Services.Services.admin_get_disbursed_loans() )
  end
  ...
  def pipeline_loans_report(conn, _params) do
    render(conn, "admin_loan_pipeline_report.html")
  end

  def loans_report_by_funder(conn, _params) do
    # funder_list = Loanmanagementsystem.Loan.list_tbl_loan_funder()
    funder_list = Loanmanagementsystem.Loan.get_funder_details()
    offtaker_list = Loanmanagementsystem.Loan.get_offtaker_details()
    render(conn, "admin_loans_report_by_funder.html", funder_list: funder_list, offtaker_list: offtaker_list)
  end


  def loans_report_by_offtaker(conn, _params) do
    offtaker_list = Loanmanagementsystem.Loan.get_offtaker_details()
    render(conn, "admin_loans_report_by_offtaker.html", offtaker_list: offtaker_list)
  end


  def loan_pipeline_report_item_lookup(conn, params) do
    {draw, start, length, search_params} = search_options(params)
    results = Loanmanagementsystem.Operations.loan__pipeline_report_miz(search_params, start, length)
    total_entries = total_entries(results)
    entries = entries(results)
  ...
  def all_disbursed_loans_report_item_lookup(conn, params) do
    {draw, start, length, search_params} = search_options(params)
    results = Loanmanagementsystem.Operations.disbursed_loans_by_funder_report_miz(search_params, start, length)
    total_entries = total_entries(results)
    entries = entries(results)
  ...
  def end_of_day__totals_view(conn, _params) do
    get_loan_funder = Loanmanagementsystem.Loan.get_loan_funder_details()
    products = Loanmanagementsystem.Products.list_tbl_products()
    render(conn, "end_of_day__totals.html", products: products, get_loan_funder: get_loan_funder, loans: Loanmanagementsystem.Services.Services.admin_get_disbursed_loans() )
  end


    def compose_end_of_day_totals_list(conn, params) do
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.Loan.end_of_day_loans_listing(search_params, start, length)
      total_entries = total_entries(results)
      entries = entries(results)
  ...
  def loan_per_offtaker_report_item_lookup(conn, params) do
    {draw, start, length, search_params} = search_options(params)
    results = Loanmanagementsystem.Operations.loan_per_offtaker_report_miz(search_params, start, length)
    total_entries = total_entries(results)
    entries = entries(results)
  ...
  def view_all_eod_entries(conn, params) do
    end_of_day_entries = Loanmanagementsystem.Loan.get_all_eod_entries_by_eod_id((params["eod_ref_no"]))
    # IO.inspect(end_of_day_entries, label: "end_of_day_entries here")
    render(conn, "view_all_loans_eod_entries.html", end_of_day_entries: end_of_day_entries)
  end


  def all_innactive_clients_lookup(conn, params) do
    {draw, start, length, search_params} = search_options(params)
    results = Loanmanagementsystem.Loan.all_innactive_clients_lookup(search_params, start, length)
    total_entries = total_entries(results)
    entries = entries(results)
  ...
  def active_loans_by_loan_officer(conn, params) do
    {draw, start, length, search_params} = search_options(params)
    results = Loanmanagementsystem.RelationshipManagement.active_loans_by_loan_officer(search_params, start, length)
    total_entries = total_entries(results)
    entries = entries(results)
  ...
  def due_collected_by_loan_officer(conn, params) do
    {draw, start, length, search_params} = search_options(params)
    results = Loanmanagementsystem.RelationshipManagement.due_collected_by_loan_officer(search_params, start, length)
    total_entries = total_entries(results)
    entries = entries(results)
  ...
  def get_over_due_loans_look_up(conn, params) do
    {draw, start, length, search_params} = search_options(params)
    results = Loanmanagementsystem.Loan.clients_overdue_loans_items_listing(search_params, start, length)
    total_entries = total_entries(results)
    entries = entries(results)
  ...
  defp reject_first_record([_first | rest]) do
    rest
  end
  defp reject_first_record([]) do
    []
  end

  def reject_first_loan(disbursed_loans) do
    reject_first_record(disbursed_loans)
  end


  def corporate_loan_client_statement_report(conn, %{"loan_id" => loan_id}) do
      ...
      disbursed_loans = Loanmanagementsystem.Operations.get_corperate_customer_statement(loan_id)
      client_details = Loanmanagementsystem.Operations.get_corperate_customer_details_for_statement(loan_id)
      repayment_totals = Loanmanagementsystem.Operations.get_corperate_customer_totals_for_statement(loan_id)
      accrued_figures = Loanmanagementsystem.Operations.get_corperate_customer_statement_accrues(loan_id)
      first_records = Loanmanagementsystem.Operations.get_corperate_customer_statement_first_record(loan_id)
      filtered_loans = LoanmanagementsystemWeb.ReportsController.reject_first_loan(disbursed_loans)
    render(conn, "corporate_client_statement.html", filtered_loans: filtered_loans, first_records: first_records, accrued_figures: accrued_figures, date: date, disbursed_loans: disbursed_loans, client_details: client_details, repayment_totals: repayment_totals)
  end



  def individual_loan_client_statement_report(conn, %{"loan_id" => loan_id}) do
      ...
      date =
        ...
        |> Calendar.DateTime.shift_zone!("Africa/Cairo")
        |> Calendar.Strftime.strftime!("%d %B, %Y")
      disbursed_loans = Loanmanagementsystem.Operations.get_individual_customer_statement(loan_id)
      client_details = Loanmanagementsystem.Operations.get_individual_customer_details_for_statement(loan_id)
      repayment_totals = Loanmanagementsystem.Operations.get_corperate_customer_totals_for_statement(loan_id)
      accrued_figures = Loanmanagementsystem.Operations.get_individual_customer_statement_accrues(loan_id)
      first_records = Loanmanagementsystem.Operations.get_individual_customer_statement_first_record(loan_id)
      filtered_loans = LoanmanagementsystemWeb.ReportsController.reject_first_loan(disbursed_loans)

    render(conn, "individual_client_statement.html", filtered_loans: filtered_loans, first_records: first_records, accrued_figures: accrued_figures, date: date, disbursed_loans: disbursed_loans, client_details: client_details, repayment_totals: repayment_totals)
  end

  def all_loans_by_funder_report(conn, _params),
  do:
    render(conn, "view_all_loans_by_funder.html", products: Loanmanagementsystem.Products.list_tbl_products())


    def all_funder_loan_report_lookup(conn, params) do
      this_year = Timex.today.year
      funderID = conn.assigns.user.id
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.Operations.all_loan_analysis_by_funder_report(search_params, start, length, this_year, funderID)
      total_entries = total_entries(results)
      entries = entries(results)
    ...
    def view_all_over_due_loans_by_funder(conn, _params),
    do:
      render(conn, "view_all_over_due_loans_by_funder.html", products: Loanmanagementsystem.Products.list_tbl_products())



    def all_funder_over_due_loan_report_lookup(conn, params) do
      this_year = Timex.today.year
      funderID = conn.assigns.user.id
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.Operations.all_over_due_loan_analysis_by_funder_report(search_params, start, length, this_year, funderID)
      total_entries = total_entries(results)
      entries = entries(results)
    ...
    def last_five_disbursed_loans_lookup_per_funder(conn, params) do
      funderID = conn.assigns.user.id
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.Operations.last_five_disbursed_loans_per_funder(search_params, start, length, funderID)
      total_entries = total_entries(results)
      entries = entries(results)
    ...
    def last_five_disbursed_loans_lookup_miz(conn, params) do
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.Operations.last_five_disbursed_loans_miz(search_params, start, length)
      total_entries = total_entries(results)
      entries = entries(results)
    ...
    def last_per_product_disbursed_lookup_per_funder(conn, params) do
      funderID = conn.assigns.user.id
      {draw, start, length, search_params} = search_options(params)
      results = Loanmanagementsystem.Operations.get_active_loans_by_product_per_funder(search_params, start, length, funderID)
      total_entries = total_entries(results)
      entries = entries(results)
    ...
    # def insert_records_if_not_exists() do
    #   # Define a list of fields you want to copy from the source table
    #   fields_to_copy = [:loanID, :customer_id]

    #   # Define the source and target table schemas
    #   source_table = Loans
    #   target_table = Loanmanagementsystem.Loan.Loan_invoice

    #   # Build a query to select records from the source table
    #   source_query =
    #     from s in source_table,
    #     select: %{^s.id => :id} ++ Map.take(s, id, customer_id),
    #     where: not ^s.id in subquery(from t in target_table, select: t.id)

    #   # Retrieve records from the source table
    #   records = Repo.all(source_query)

    #   # Insert records into the target table
    #   for %{} = record <- records do
    #     target_query = insert(target_table, %{
    #       ^record | id: nil  # Set id to nil to trigger an auto-increment
    #     })

    #     {:ok, _} = Repo.insert(target_query)
    #   end

    def insert_invoice_records_if_not_exist() do
      query =
        from(l in Loans,
          left_join: li in Loanmanagementsystem.Loan.Loan_invoice,
          on: l.id == li.loanID,
          where: is_nil(li.id) and l.loan_type == "INVOICE DISCOUNTING",
          select: l
        )
      ...
      records_to_insert =
        ...
        Enum.each(records_to_insert, fn record ->
          changeset = Loanmanagementsystem.Loan.Loan_invoice.changeset(%Loanmanagementsystem.Loan.Loan_invoice{}, %{
            loanID: record.id,
            customer_id: record.customer_id,  # Set appropriate values for other_fields
            status: "ACTIVE",
    ...
    # end  LoanmanagementsystemWeb.ReportsController.insert_order_finance_records_if_not_exist()

    def insert_order_finance_records_if_not_exist() do
      query =
        from(l in Loans,
          left_join: li in Loanmanagementsystem.Loan.Order_finance_loan_invoice,
          on: l.id == li.loan_id,
          where: is_nil(li.id) and l.loan_type == "ORDER FINANCE",
          select: l
        )
      ...
      records_to_insert =
        ...
        Enum.each(records_to_insert, fn record ->
          changeset = Loanmanagementsystem.Loan.Order_finance_loan_invoice.changeset(%Loanmanagementsystem.Loan.Order_finance_loan_invoice{}, %{
            loan_id: record.id,
            customer_id: record.customer_id,  # Set appropriate values for other_fields
            status: "ACTIVE",
            order_date: record.application_date,
