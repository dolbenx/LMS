defmodule LoanmanagementsystemWeb.FinancialManagementController do
  ...
  def search_transaction(conn, _params), do: render(conn, "search_transaction.html")
  def freqeunt_posting(conn, _params), do: render(conn, "frequent_postings.html")

  def financial_activity_mapping(conn, _params),
    do: render(conn, "financial_activity_mapping.html")

  def fund_association(conn, _params), do: render(conn, "fund_association.html")
  def migrate_opening_balances(conn, _params), do: render(conn, "migrate_opening_balances.html")
  def acruals(conn, _params), do: render(conn, "acruals.html")
  def provisioning_entries(conn, _params), do: render(conn, "provisioning_entries.html")
  def closing_entries(conn, _params), do: render(conn, "closing_entries.html")
