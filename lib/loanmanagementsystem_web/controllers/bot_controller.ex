defmodule LoanmanagementsystemWeb.BotController do
  ...
  def welcome_menu(conn, _params, query_params) do
    response =
  ...
  # https://openexchangerates.org/api/convert/{amount}/{from_currency}/{to_currency}?app_id=YOUR_APP_ID


  # LoanmanagementsystemWeb.BotController.convert(100, "USD")
  def convert(amount, from_currency) do
    url = build_url(from_currency)
    response = HTTPoison.get(url)

    status_code =
      with {:ok, %HTTPoison.Response{status_code: code}} <- response do
        code
      end

    body =
      with {:ok, %HTTPoison.Response{body: code}} <- response do
        code
      end

    data = Jason.decode!(body)
    rates = data["rates"]
    zmw_value = Map.get(rates, "ZMW")

    case status_code do
      200 ->
        converted_amount = amount / zmw_value
        {:ok, converted_amount}
        IO.inspect(converted_amount, label: "Hello Dollar")
  ...
  defp build_url(from_currency) do
    "#{@base_url}#{from_currency}"
  end

  # defp parse_response(body, to_currency) do
  #   rates = Jason.decode!(body)["conversion_rates"]
  #   rates[to_currency]
  # end
