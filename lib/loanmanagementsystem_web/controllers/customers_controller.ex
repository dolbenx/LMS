defmodule LoanmanagementsystemWeb.CustomersController do
  ...
  plug LoanmanagementsystemWeb.Plugs.Authenticate,
        [module_callback: &LoanmanagementsystemWeb.CustomersController.authorize_role/1]
        when action not in [
          :admin_loan_repayment,
          :calculate_page_num,
          :calculate_page_size,
          :company,
          :customer_loan_item_lookup,
          :entries,
          :individuals,
          :init,
          :oct_customer_lookup,
          :oct_loan_application,
          :oct_product_lookup,
          :offtaker,
          :otc,
          :otc_logs,
...
use PipeTo.Override
  ...
  def update_user_data(conn, %{"user_id" => user_id} = params) do
    ...
    |> case do
      ...
      {:ok, %{update: _update, insert: _insert}} ->
        client_upload(%{"process_documents" => params, "conn" => conn, "nrc" => user_details.meansOfIdentificationNumber})
        conn
  ...
  def client_upload(%{"process_documents" => files, "conn" => conn, "nrc" => nrc }) do
    IO.inspect(files, label: "~~~~~~~~~~~~~")

    generate_path(conn)
    |> case do
      {:ok, path} ->
        handle_documents(%{conn: conn, path: path, files: files,  nrc: nrc,  file_category: "ACCOUNT COMPLETION (EMPLOYEE DASHBOARD)"})
  ...
  defp new_route(conn) do
    # uuid = Ecto.UUID.generate()
    company_id = conn.assigns.user.company_id
    date_path = Date.utc_today()
    day = date_path.day

    "/company/#{company_id}/employee/docs/#{date_path.year}-#{date_path.month}-#{day}"
  end

  def handle_documents(%{conn: conn, path: path, files: files,  nrc: nrc,  file_category: file_category}) do
  ...
  def handle_uploaded_files(request, conn) do
    ...
    # customer_id = if conn.assigns.user == do
    #   true -> conn.assigns.user.id
    #   false-> conn.assigns.current_user.id
    # end
