...
use PipeTo.Override
  ...
  def admin_create_country(conn, params) do
    ...
    case is_nil(get_code) do
      ...
      _ ->
        conn
        |> put_flash(:error, "County #{name} already exists")
        |> redirect(to: Routes.system_management_path(conn, :countries))
      end

    _ ->
      conn
      |> put_flash(:error, "County code #{code} already exists")
      |> redirect(to: Routes.system_management_path(conn, :countries))
    end
  end
  ...
  def admin_create_province(conn, params) do
    name = params["name"]
    get_name = Repo.get_by(Province, name: name)
    ...
    _ ->
      conn
      |> put_flash(:error, "Provice name #{name} already exists")
      |> redirect(to: Routes.system_management_path(conn, :province))
    end
  end
  ...
  def admin_create_district(conn, params) do
    name = params["name"]
    get_name = Repo.get_by(District, name: name)
    ...
    _ ->
      conn
        |> put_flash(:error, "City name #{name} already exists")
        |> redirect(to: Routes.system_management_path(conn, :district))
    end
  end
    ...
    case path != nil do
      true ->
        with {_ok, doc} <- validate_document(docx) do
          ...
          |> Ecto.Multi.run(:upload, fn _REPO_, %{docx: docx} ->
