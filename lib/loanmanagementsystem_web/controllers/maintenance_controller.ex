...
use PipeTo.Override
  ...
  def add_charge(conn, params) do
    ...
    |> Ecto.Multi.run(:user_logs, fn _repo, %{charges: charges} ->
      UserLogs.changeset(%UserLogs{}, %{
        ...
        user_id: conn.assigns.user.idadd_charge
      })
      |> Repo.insert()
    end)
    |> Repo.transaction()
  ...
  def admin_user_logs(conn, _params) do
    user_logs = Loanmanagementsystem.Operations.admin_user_logs
    render(conn, "admin_user_logs.html", user_logs: user_logs)
  end
  ...
  def create_bank(conn, params) do
    bank_details_ = String.split(params["process_branch"], "|||")

    params =
      Map.merge(params, %{
        "center_code" => Enum.at(bank_details_, 0),
        "process_branch" => Enum.at(bank_details_, 1)
      })
  ...
  def admin_update_province(conn, params) do
    ...
    |> case do
      {:ok, _} ->
        conn
        |> put_flash(:info, "You have Successfully Added a Province")
        |> redirect(to: Routes.maintenance_path(conn, :province))
