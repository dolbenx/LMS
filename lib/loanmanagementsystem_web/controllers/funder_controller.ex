defmodule LoanmanagementsystemWeb.FunderController do
  ...
  def funder_dashboard(conn, _params) do
      ...
      loans_funds_count = Loanmanagementsystem.Loan.funder_dashboard_analysis(funder_id)
      last_five_loans = Loanmanagementsystem.Loan.get_last_five_disbursed_loans(funder_id)
      active_loans_by_prod = Loanmanagementsystem.Loan.get_active_loans_by_product_per_funder(funder_id)
      get_repaid_loans_by_product =  Loanmanagementsystem.Loan.get_repaid_loans_by_product_per_funder(funder_id)
      get_over_due_loans_by_product =  Loanmanagementsystem.Loan.get_over_due_loans_by_product_per_funder(funder_id)
      render(conn, "funder_dashboard.html", get_over_due_loans_by_product: get_over_due_loans_by_product, get_repaid_loans_by_product: get_repaid_loans_by_product, active_loans_by_prod: active_loans_by_prod, last_five_loans: last_five_loans, loans_funds_count: loans_funds_count, funds_count: funds_count, count_loans_by_funder: count_loans_by_funder)
  end
  ...
  # Loanmanagementsystem.Loan.get_corparate_loan_by_funder_id(106)
  ...
  def approve_funder_invoice_discounting(conn, %{"loan_id" => loan_id}) do
    ...
    render(conn, "approve_invoice_discounting_loan.html", loan_details: loan_details, loanDocs: loanDocs, credit_loan_details: credit_loan_details, funder_amounts: funder_amounts, loan_invoice_details: loan_invoice_details)
  end
  ...
  def view_funder_invoice_discounting(conn, %{"loan_id" => loan_id}) do
    ...
    render(conn, "invoice_discounting_funder_view.html", loan_details: loan_details, loanDocs: loanDocs, loan_invoice_details: loan_invoice_details)
  end

  def approve_invoice_discounting_application(conn, params) do

    disbursed_amount = params["disbursement_amount"]

    total_funder_balance = params["totalbalance"]

      disbursed_amount_convert = try do
        case String.contains?(String.trim(disbursed_amount), ".") do
          true ->  String.trim(disbursed_amount) |> String.to_float()
          false ->  String.trim(disbursed_amount) |> String.to_integer() end
        rescue _-> 0 end

      total_funder_balance_convert = try do
        case String.contains?(String.trim(total_funder_balance), ".") do
          true ->  String.trim(total_funder_balance) |> String.to_float()
          false ->  String.trim(total_funder_balance) |> String.to_integer() end
        rescue _-> 0 end

    if  disbursed_amount_convert <=  total_funder_balance_convert do

      requested_amount = params["amount"]

      requested_amount_convert = try do
      ...
      total_funder_amount = total_funder_balance_convert

      requested_amount = requested_amount_convert

      total_loan_recieved = disbursed_amount_convert
  ...
  def funder_all_documents_for_loans(conn, params) do
    documents = Loanmanagementsystem.Operations.get_loan_funder_docs(conn.assigns.user.id, params["loan_id"])
