defmodule LoanmanagementsystemWeb.SmeController do
  ...
  def sme_update_360_profile(conn, params) do
    update_360 = Loanmanagementsystem.Accounts.get_user_bio_data!(params["id"])
    Ecto.Multi.new()
  ...
  def sme_update_offtaker(conn, params) do
    ...
    |> case do
      {:ok, _msg} ->
        ...
        |> redirect(to: Routes.sme_path(conn, :sme_off_taker))
  ...
  def sme_mini_statement(conn, _request) do
    render(conn, "mini_statement.html")
  end
  ...
  def sme_loan_product(conn, _request) do
