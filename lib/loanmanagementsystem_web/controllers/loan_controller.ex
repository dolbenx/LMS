defmodule LoanmanagementsystemWeb.LoanController do
  ...
  #             |> Repo.transaction()
  #             |> case do
  #               {:ok, %{add_loan: _add_loan, user_logs: _user_logs, colleteral_document: _colleteral_document}} ->
...
def executive_committe_loan_approval(conn, params) do
  ...
  def approve_employee_consumer_loan_by_management(conn, params) do
    ...
    get_amount_proposed_1 = params["amount_proposed"] #this is the employee account balance
    ...
    get_amount_proposed = try do
      case String.contains?(String.trim(get_amount_proposed_1), ".") do
        true ->  String.trim(get_amount_proposed_1) |> String.to_float()
        false ->  String.trim(get_amount_proposed_1) |> String.to_integer() end
    rescue _-> 0 end
    ...
    IO.inspect(get_amount_proposed, label: "------------------------PROPOSED-----------------")
    IO.inspect(limit_balance, label: "------------------------PROPOSED-----------------")

    if get_amount_proposed <= get_loan_limit_balance do
    ...
    Ecto.Multi.new()
      ...
      |> Ecto.Multi.run(:update_employee_account, fn _repo, %{update_loan: _update_loan} ->
      ...
      |> case do
        ...
        conn
        |> put_flash(:error, "This loan can not be approved because the amount proposed is more than the maximum loan limit. Your limit balance is K#{get_loan_limit_balance}")
        |> redirect(to: Routes.loan_path(conn, :consumer_loans_datatable))
      end
  end
