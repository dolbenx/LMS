defmodule LoanmanagementsystemWeb.LoanController do
  ...
  plug LoanmanagementsystemWeb.Plugs.Authenticate,
          ...
          when action not in [
            ...
            :approved_by_senior_loan_officer,
            :disbursed_by_finance_loan_officer,
            :disburse_finance_loan_officer
  ...
  ################################################################################### INVOICE DISCOUNTING #######################################################################################
    ...
    def all_product_loan_repayment(conn, params) do
      ...
      |> Ecto.Multi.insert(:repayment, LoanRepayment.changeset(%LoanRepayment{}, %{
        amountRepaid: params["repayment_amount"],
      ...
      }))|> IO.inspect(label: "iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii")
      ...
      |> IO.inspect(label: "ccccccccccccccccccccccccccccccccccccccccccccccccccc")
    ...
    def create_invoice_discounting_application(conn, params) do
      ...
      if params["has_mou"] == "NO" do
        ...
        calculated_invoiceValue = String.to_integer(params["invoiceValue"]) * 0.6


        if calculated_invoiceValue >= String.to_integer(params["requested_amount"]) do
          ...
          # reference_no = generate_momo_random_string(10)
            ...
            conn
            |> put_flash(:error, "You only qualify for 60% of your invoice value, Please check and try again.")
            |> redirect(to: Routes.loan_path(conn, :invoice_discounting_application_datatable))
        end
      end
    end
    ...
    def push_no_mou_create_invoice_discounting_application(conn, params) do
      ...
      calculated_invoiceValue = String.to_integer(params["invoiceValue"]) * 0.7
    ...
    if calculated_invoiceValue >= String.to_integer(params["requested_amount"]) do
      ...
      if params["customer_id"] != "" || params["customer_id"] != nil do
        ...
        get_bio_data = try do Loanmanagementsystem.Accounts.UserBioData.find_by(id: get_bio_id) rescue _-> nil end
          ...
          |> Repo.transaction()
          |> case do
            {:ok, %{add_loan: add_loan, user_logs: _user_logs, loan_invoice: _loan_invoice, document: _document}} ->
              # Email.confirm_approval(email_address)
              # Loanmanagementsystem.Workers.Sms.send()
        ...
        conn
        |> put_flash(:error, "You only qualify for 70% of your invoice value, Please check and try again.")
        |> redirect(to: Routes.loan_path(conn, :invoice_discounting_application_datatable))
      end
    end
    ...
    def approve_credit_invoice_discounting(conn, %{"loan_id" => loan_id}) do
      ...
      render(conn, "approve_credit_invoice_discounting.html", loan_details: loan_details, loanDocs: loanDocs, credit_loan_details: credit_loan_details, funder_details: funder_details)
    end
    ...
    def approve_accounts_invoice_discounting(conn, %{"loan_id" => loan_id}) do
      ...
      render(conn, "approve_accounts_invoice_discounting.html", loan_details: loan_details, loanDocs: loanDocs, credit_loan_details: credit_loan_details)
    end
    ...
    def admin_approve_operations_ordering_finance(conn, %{"loan_id" => loan_id}) do
      ...
      render(conn, "admin_approve_operations_ordering_finance.html", loan_details: loan_details, loanDocs: loanDocs, credit_loan_details: credit_loan_details, offtaker_details: offtaker_details)
    end
    ...
    def view_invoice_discounting(conn, %{"loan_id" => loan_id}) do
      ...
      render(conn, "invoice_discounting_view.html", loan_details: loan_details, loanDocs: loanDocs)
    end
    ...
    def mgt_view_invoice_discounting(conn, %{"loan_id" => loan_id}) do
      ...
      render(conn, "mgt_invoice_discounting_view.html", get_user_details: get_user_details, loan_details: loan_details, loanDocs: loanDocs)
    end
  ...
  def order_finance_application_datatable(conn, params),
    do: render(conn, "order_finance_loans_table.html")


    def sme_loan_application_datatable(conn, params),
  ...
  def universal_loan_application_capturing(conn, params) do
   ...
   product_type =  try do Loanmanagementsystem.Products.Product.find_by(id: params["product_id"]).productType rescue _-> nil end
    if product_type == "CONSUMER LOAN" do
      ...
      if product_type == "SALARY ADVANCE" do
        nrc = params["nrc"]
        client_data = Loanmanagementsystem.Accounts.get_client_by_nrc(nrc)
        customers_address = try do Loanmanagementsystem.Accounts.get_client_address_details(client_data.userId) ||
        %{
        ...
        customers_income = Loanmanagementsystem.Employment.list_income_details(client_data.userId)
        customers_employment = Loanmanagementsystem.Employment.list_employement(client_data.userId)
        account_details = Loanmanagementsystem.Companies.list_employee_account_details(client_data.userId)
        nextofkin = try do  Loan.list_customer_nextofkin_details(client_data.userId) ||
          %{
            applicant_nrc: "",
            kin_ID_number: "",
    ...
    get_bio_data = try do Loanmanagementsystem.Accounts.UserBioData.find_by(meansOfIdentificationNumber: params["id_number"]) rescue _-> nil end
      ...
      |> case do
        {:ok, %{add_loan: _add_loan, user_logs: _user_logs, document: _document}} ->
          ...
          # Loanmanagementsystem.Workers.Sms.send()
          conn
          |> put_flash(:info, "Loan application has successfully been submitted")
    ...
    if product_type == "CONSUMER LOAN" do
      ...
      if product_type == "SALARY ADVANCE" do
        with(
          ...
          get_account_details = Companies.list_employee_account_details(params["userId"])
...
def approved_by_senior_loan_officer(conn, params) do

  current_user = conn.assigns.user.id
  client_details = Loanmanagementsystem.Loan.Loan_customer_details.find_by(customer_id:  params["userId"], reference_no: params["reference_no"])
  nrc = client_details.id_number
  positions = Loanmanagementsystem.Operations.get_current_user_by_bio_data(current_user)
  current_user_role_id= Loanmanagementsystem.Accounts.User.find_by(id: current_user).role_id
  current_user_role = Loanmanagementsystem.Accounts.Role.find_by(id: current_user_role_id).role_group
  ...
  render(conn, "approved_by_senior_loan_officer.html", positions: positions, current_user_role_id: current_user_role_id, current_user_role: current_user_role, client_address_details: client_address_details, get_user_details: get_user_details, loanDocs: loanDocs, client_data: client_data, client_kyc: client_kyc, nextofkin: nextofkin, client_references: client_references, loan_details: loan_details, collateral_details: collateral_details, extracted_other_collateral_details: extracted_other_collateral_details, sales_recommedation: sales_recommedation, market_info: market_info, get_account_details: get_account_details, credit_analyst_recommedation: credit_analyst_recommedation, credit_manager_recommedation: credit_manager_recommedation, accounts_assistant_recommedation: accounts_assistant_recommedation, finance_manager_recommedation: finance_manager_recommedation, product_details: product_details, product_rate_details: product_rate_details, account_details: account_details)
end

def disbursed_by_finance_loan_officer(conn, params) do
  ...
  render(conn, "disbursed_by_finance_loan_officer.html", positions: positions, current_user_role_id: current_user_role_id, current_user_role: current_user_role, client_address_details: client_address_details, get_user_details: get_user_details, loanDocs: loanDocs, client_data: client_data, client_kyc: client_kyc, nextofkin: nextofkin, client_references: client_references, loan_details: loan_details, collateral_details: collateral_details, extracted_other_collateral_details: extracted_other_collateral_details, sales_recommedation: sales_recommedation, market_info: market_info, get_account_details: get_account_details, credit_analyst_recommedation: credit_analyst_recommedation, credit_manager_recommedation: credit_manager_recommedation, accounts_assistant_recommedation: accounts_assistant_recommedation, finance_manager_recommedation: finance_manager_recommedation, product_details: product_details, product_rate_details: product_rate_details, account_details: account_details)
end

def disburse_finance_loan_officer(conn, params) do

  total_loan_recieved_1 = params["disbursement_amount"]

  get_principal_1 = params["amount"] #this is the actual loan amount
  get_account_balance_1 = params["account_balance"] #this is the employee account balance
  # get_loan_limit_1 = params["loan_limit"] #this is the employee account balance
  get_loan_limit_balance_1 = params["loan_limit_balance"] #this is the employee account balance


  get_principal = try do
    case String.contains?(String.trim(get_principal_1), ".") do
      true ->  String.trim(get_principal_1) |> String.to_float()
      false ->  String.trim(get_principal_1) |> String.to_integer() end
  rescue _-> 0 end

  get_account_balance = try do
    case String.contains?(String.trim(get_account_balance_1), ".") do
      true ->  String.trim(get_account_balance_1) |> String.to_float()
      false ->  String.trim(get_account_balance_1) |> String.to_integer() end
  rescue _-> 0 end

  get_loan_limit_balance = try do
    case String.contains?(String.trim(get_loan_limit_balance_1), ".") do
      true ->  String.trim(get_loan_limit_balance_1) |> String.to_float()
      false ->  String.trim(get_loan_limit_balance_1) |> String.to_integer() end
  rescue _-> 0 end

  limit_balance = (get_loan_limit_balance - get_principal)

  IO.inspect(get_principal_1, label: "------------------------PROPOSED-----------------")
  IO.inspect(limit_balance, label: "------------------------PROPOSED-----------------")

  new_account_balance = (get_principal + get_account_balance)
  ...
  total_loan_recieved = try do
    case String.contains?(String.trim(total_loan_recieved_1), ".") do
      true ->  String.trim(total_loan_recieved_1) |> String.to_float()
      false ->  String.trim(total_loan_recieved_1) |> String.to_integer() end
  rescue _-> 0 end

  # reference_no = generate_momo_random_string(10)

  loans = Loanmanagementsystem.Loan.get_loan_by_id!(params["id"])
  account_id = try do Employee_account.find_by(employee_number: params["employee_number"]).id rescue _-> nil end
  update_employee_account = Loanmanagementsystem.Companies.get_employee_account!(account_id)
  ...
  case total_loan_recieved do
    ...
    total_loan_recieved when total_loan_recieved <= requested_amount ->
      ...
      |> Ecto.Multi.run(:update_employee_account, fn _repo, %{add_loan: _add_loan} ->
        Employee_account.changeset(update_employee_account, %{
      ...
      |> Ecto.Multi.run(:sms, fn _, %{add_loan: _add_loan, update_employee_account: update_employee_account} ->
        sms = %{
          mobile: params["phone_number"],
          msg:
          "Dear #{params["full_name"]}, Your Loan Application has been approved and the amount of K#{params["amount"]} has been disbursed into your account.",
          status: "READY",
          type: "SMS",
          msg_count: "1"
        }
      ...
      |> Ecto.Multi.run(:document, fn _repo, %{add_loan: add_loan, user_logs: _user_logs} ->
        Loanmanagementsystem.Services.SalaryAdavanceUploads.salary_advance_upload(%{"conn" => conn, "customer_id" => params["customer_id"], "loan_id" => add_loan.id}, params)
      end)
      |> Repo.transaction()
      |> case do
        {:ok, %{add_loan: _add_loan, user_logs: _user_logs, document: _document}} ->
          Loanmanagementsystem.Workers.Sms.send()
          conn
          |> put_flash(:info, "Loan Application Disbursed Successfully By Finance Officer")
          |> redirect(to: Routes.loan_path(conn, :consumer_loans_datatable))
...
def executive_committe_approving_loan_application_view(conn, params) do
  current_user = conn.assigns.user.id
  product_type =  try do Loanmanagementsystem.Products.Product.find_by(id: params["product_id"]).productType rescue _-> nil end
  IO.inspect(product_type, label: "-----------------------ID-----------------")
  ...
  if product_type == "CONSUMER LOAN" do
    ...
    if product_type == "SALARY ADVANCE" do
      with(
        ...
        _ ->
        conn
        |> put_flash(:error, "eeeeeeeeeeeeeeeeeeeeee")
        |> redirect(to: Routes.credit_management_path(conn, :loan_appraisal))
      end
...
def executive_committe_loan_approval(conn, params) do
  ...
  def company_otp_validation(conn, %{"client_line" => client_line, "product_id" => product_id, "registrationNumber" => registrationNumber} = params) do
        ...
        |> Ecto.Multi.run(:user_logs, fn _repo, %{loan_otp: _loan_otp} ->
          ...
          |> case do
            {:ok, %{loan_otp: _loan_otp, user_logs: _user_logs}} ->
              Loanmanagementsystem.Workers.Sms.send()
              conn
              |> put_flash(:info, "OTP has been sent to company company phone Number")
  ...
  def generate_schedule(loan_amount_v, interet_rate_v, term_in_months_v, start_date) do
  ...
  def approve_employee_consumer_loan_by_accountant(conn, params) do
    ...
    maturity_date = Date.add(Timex.today(), loan_details.tenor)
      ...
      |> Ecto.Multi.run(:user_logs, fn _repo, %{update_loan: update_loan} ->
        UserLogs.changeset(%UserLogs{}, %{
          activity: "Loan Application Successfully Approved By Accountant",
          user_id: conn.assigns.user.id
        })
  ...
  def approve_employee_consumer_loan_by_management(conn, params) do
    ...
    Ecto.Multi.new()
      |> Ecto.Multi.update(
        :update_loan,
        Loans.changeset(loans, %{
          loan_status: "PENDING_FINANCE_OFFICER",
          status: "PENDING_FINANCE_OFFICER",
          approvedon_date: params["approvedon_date"],
          approvedon_userid: conn.assigns.user.id,
          disbursedon_date: Timex.today(),
          principal_disbursed_derived: params["amount"],
      ...
      |> Ecto.Multi.run(:recomendation, fn _repo, %{update_loan: _update_loan} ->
        Loan_recommendation_and_assessment.changeset(%Loan_recommendation_and_assessment{}, %{
          comments: params["comment"],
          customer_id: params["customer_id"],
          date: "#{Date.utc_today()}",
      ...
      |> Ecto.Multi.run(:user_logs, fn _repo, %{update_loan: _update_loan} ->
        UserLogs.changeset(%UserLogs{}, %{
          activity: "Loan Application Successfully Approved",
          user_id: conn.assigns.user.id
        })
        |> Repo.insert()
      ...
      |> case do
        {:ok, %{update_loan: _update_loan, user_logs: _user_logs}} ->
          # Loanmanagementsystem.Services.InvoiceUploads.invoice_loan_upload(%{conn: conn, customer_id: params["customer_id"], loan_id: update_loan.id}, params)
          conn
          |> put_flash(:info, "Loan application approved and successfully sent to finance officer for disbursement")
          |> redirect(to: Routes.loan_path(conn, :consumer_loans_datatable))
...
#  |> Enum.reject(&(&1.status != "ACTIVE"))
  ...
  def resend_otp(conn, %{"client_line" => client_line, "nrc" => nrc, "product_id" => product_id} = params) do
    ...
    # params = Map.put(params, "client_line", client_line)
    # params = Map.put(params, "nrc", nrc)
    # params = Map.put(params, "product_id", product_id)

    resend_fxn = LoanmanagementsystemWeb.LoanController.send_otp(conn, params)

    # IO.inspect(client_line, label: "CHeck line")
    # get_user_bio = Loanmanagementsystem.Accounts.UserBioData.find_by(mobileNumber: client_line)
    # get_user_role = Loanmanagementsystem.Accounts.UserRole.find_by(userId: get_user_bio.userId)
    # text = "To verify your loan initiation, please provide the OTP - #{get_user_role.otp}"
    ...
    #   Ecto.Multi.new()
    #     |> Ecto.Multi.insert(:loan_otp, Sms.changeset(%Sms{}, new_params))
    #     |> Ecto.Multi.run(:user_logs, fn _repo, %{loan_otp: _loan_otp} ->
    #       UserLogs.changeset(%UserLogs{}, %{
    #         activity: "Customer OTP Successfully sent",
    #         user_id: conn.assigns.user.id
    ...
    #           Loanmanagementsystem.Workers.Sms.send()
    #           conn
    #           |> put_flash(:info, "OTP has been resent to Mobile Number #{client_line}")
    #           |> redirect(to: Routes.loan_path(conn, :otp_validation, client_line: "#{client_line}", product_id: "#{product_id}", nrc: "#{nrc}"))
  ...
  def resend_company_otp(conn, %{"client_line" => client_line, "registrationNumber" => registrationNumber, "product_id" => product_id} = params) do
    ...
    # client_rec = Loanmanagementsystem.Companies.get_company_by_regNo(registrationNumber)
    # # get_user_bio = Loanmanagementsystem.Accounts.UserBioData.find_by(userId: client_rec.userId)

    # get_user_role = Loanmanagementsystem.Accounts.UserRole.find_by(userId: client_rec.userId)
    # IO.inspect(get_user_role, label: "CHeck get_user_role")
    # text = "To verify your loan initiation, please provide the OTP - #{get_user_role.otp}"

    #     new_params = %{
    #       mobile: client_rec.companyPhone,
    #       status: "READY",
    #       type: "SMS",
    #       msg_count: "1",
    #       msg: text,
    ...
    #           Loanmanagementsystem.Workers.Sms.send()
    #           conn
    #           |> put_flash(:info, "OTP has been resent to Mobile Number #{client_rec.companyPhone}")
    #           |> redirect(to: Routes.loan_path(conn, :company_otp_validation, client_line: "#{client_line}", product_id: "#{product_id}", registrationNumber: "#{registrationNumber}"))
...
# LoanmanagementsystemWeb.LoanController.post_amortazation("50000", "9", "3", Timex.today, "6", "3", "1234")
  def post_amortazation(loan_amount_v, interet_rate_v, term_in_months_v, calculation_date, loan_id, customer_id, reference_no) do
    ...
    Ecto.Multi.new()
      ...
      |> case do
        {:ok, %{update_loan: _update_loan, user_logs: _user_logs}} ->
          # Loanmanagementsystem.Services.InvoiceUploads.invoice_loan_upload(%{conn: conn, customer_id: params["customer_id"], loan_id: update_loan.id}, params)
          conn
          |> put_flash(:info, "Guarantor Form and Facility Agreement Successfully uploaded")
          |> redirect(to: Routes.loan_path(conn, :consumer_loans_datatable))
