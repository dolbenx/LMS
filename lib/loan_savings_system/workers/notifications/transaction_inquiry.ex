					...
					{:ok, struct} ->
						...
						if(status_code=="TS") do
								...
								if(fixedDeposit) do
									...
									naive_datetime = Timex.now
										...
										sms = %{
											...
											msg: "Dear #{fixedDeposit.customerName},\nYour deposit of #{fixedDeposit.currency} #{fixedDeposit.principalAmount} has been recorded successfully. On confirmation, your deposit will be fixed for a period of #{fixedDeposit.fixedPeriod} #{fixedDeposit.fixedPeriodType} yielding you an interest of #{fixedDeposit.currency} #{fixedDeposit.expectedInterest} \nOrder Ref: #{txn.orderRef}",
											status: "READY",
											type: "SMS",
											msg_count: "1",
							...
							if(status_code=="TF") do
								...
								attrs = %{status: "Failed", referenceNo: airtelRefNo}
								txn
								|> LoanSavingsSystem.Transactions.Transaction.changesetForUpdate(attrs)
								|> Repo.update()
