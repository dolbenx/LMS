...
IO.inspect params
  ...
  # LoanSavingsSystem.FixedDeposit.fixed_deposits_report(nil, 1, 10)



  def fixed_deposits_report(search_params, page, size) do
    LoanSavingsSystem.FixedDeposit.FixedDeposits
...
# fixedPeriod
  defp compose_fixed_deposits_report_select(query) do
    ...
    |> join(:left, [fxd], div in LoanSavingsSystem.Divestments.Divestment, on: fxd.id == div.fixedDepositId)
    |> join(:left, [fxd, div], pro in LoanSavingsSystem.Products.Product, on: fxd.productId   == pro.id)
    |> join(:left, [fxd, div, pro], user in LoanSavingsSystem.Accounts.Account, on: fxd.accountId  == user.id)
    |> join(:left, [fxd, div, pro, user], dep_txn in LoanSavingsSystem.FixedDeposit.FixedDepositTransaction, on: fxd.id == dep_txn.fixedDepositId)
    |> join(:left, [fxd, div, pro, user, dep_txn], user_bio in LoanSavingsSystem.Client.UserBioData, on: fxd.userId == user_bio.userId)
    |> select(
