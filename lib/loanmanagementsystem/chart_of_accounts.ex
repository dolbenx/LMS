defmodule Loanmanagementsystem.Chart_of_accounts do
  ...
  def list_loan_details do
    Loans
    |> join(:left, [la], uB in "tbl_user_bio_data", on: la.customer_id == uB.userId)
    |> where([la, uB, uR], la.loan_status == "DISBURSED")
    |> select([la, uB, uR], %{
      id: la.id,
      ...
      idno: uB.meansOfIdentificationNumber,
    })
    |> Repo.all()
  end

  def list_loan_details_customer_accounts do
    UserBioData
    |> join(:left, [uB], la in "tbl_loans", on: la.customer_id == uB.userId)
    |> where([uB, la], la.loan_status == "DISBURSED")
    |> select([uB, la], %{
      name: fragment("select distinct case when company_name is null then concat(?, concat(' ', ?)) else company_name end from tbl_loan_customer_details where reference_no = ? ", uB.firstName, uB.lastName, la.reference_no),
      customer_id: uB.userId,
      idno: uB.meansOfIdentificationNumber,
    })
    |> group_by([uB, la], [uB.meansOfIdentificationNumber, uB.userId, uB.firstName, uB.lastName, uB.otherName, la.reference_no])
    |> Repo.all()
  end

  def list_all_customer_accounts do
    UserBioData
    |> join(:left, [uB], la in "tbl_loans", on: la.customer_id == uB.userId)
    |> join(:left, [uB, la], ta in "tbl_trans_log", on: la.customer_id == ta.customer_id)
    |> where([uB, la, ta], ta.process_status == "APPROVED")
    |> select([uB, la, ta], %{
      name: fragment("select distinct case when company_name is null then concat(?, concat(' ', ?)) else company_name end from tbl_loan_customer_details where reference_no = ? ", uB.firstName, uB.lastName, la.reference_no),
      customer_id: uB.userId,
      idno: uB.meansOfIdentificationNumber,
    ...
    |> group_by([uB, la, ta], [uB.meansOfIdentificationNumber, uB.userId, uB.firstName, uB.lastName, uB.otherName, la.reference_no, la.id])
    |> distinct(true)
    |> Repo.all()
  end
