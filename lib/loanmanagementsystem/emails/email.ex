defmodule Loanmanagementsystem.Emails.Email do
  ...
  def confirm_approval(email_address) do
    new_email()
    |> from("reports@probasegroup.com")
    |> to("#{email_address}")
    |> put_html_layout({LoanmanagementsystemWeb.LayoutView, "email.html"})
    ...
    |> render("confirm_approval.html")
    |> Mailer.deliver_later()
  end

  def send_to_client_approval(email_address) do
    new_email()
    |> from("reports@probasegroup.com")
    |> to("#{email_address}")
    |> put_html_layout({LoanmanagementsystemWeb.LayoutView, "email.html"})
    |> subject("Loan Approval Notification")
    |> assign(:email_address, email_address)
    |> render("send_to_client_approval.html")
    |> Mailer.deliver_later()
  end
  ...
  def notify_next_approver(user_bio_email, company_name_email, reference_number, requested_amount, company_tax_number, company_contact) do
    new_email()
    |> from("reports@probasegroup.com")
    |> to("#{user_bio_email}")
    ...
    |> put_html_layout({LoanmanagementsystemWeb.LayoutView, "email.html"})
    |> subject("Approve Loan")
    |> assign(:company_name_email, company_name_email)
    |> assign(:reference_number, reference_number)
    |> assign(:requested_amount, requested_amount)
    |> assign(:company_tax_number, company_tax_number)
    |> assign(:company_contact, company_contact)
    |> render("notify_next_approver.html")
    |> Mailer.deliver_later()
  end

  def email_notify_next_approver_consumer(user_bio_email, client_name, reference_number, requested_amount, client_id_number, client_contact) do
    new_email()
    |> from("reports@probasegroup.com")
    |> to("#{user_bio_email}")
    |> subject("Approve Loan")
    ...
    |> assign(:reference_number, reference_number)
    |> assign(:requested_amount, requested_amount)
    |> assign(:client_id_number, client_id_number)
    |> assign(:client_contact, client_contact)
    |> render("notify_next_approver_consumer.html")
    |> Mailer.deliver_later()
  end
  ...
  # Loanmanagementsystem.Emails.Email.send_daily_eod_entries_report(2)

  def send_daily_eod_entries_report(recipient_emails, eod_ref_no) do
    end_of_day_entries = Loanmanagementsystem.Loan.get_all_eod_entries_by_eod_id(eod_ref_no)
    sender = Loanmanagementsystem.Email_configs.list_tbl_email_sender() |>List.first()
    new_email()
    |> from(sender.email)
    |> to("#{recipient_emails.email}")
    |> subject("Daily EOD Entries Report")
    |> assign(:end_of_day_entries, end_of_day_entries)
    |> put_html_layout({LoanmanagementsystemWeb.LayoutView, "email.html"})
    |> subject("EOD")
    |> render("eod_test.html")
    |> Mailer.deliver_later()
  end

  # "kabumbwerussell@gmail.com"

  # Loanmanagementsystem.Emails.Email.send_emails_for_existing_users("kabumbwerussell@gmail.com", "Test")

  def send_emails_for_existing_users(recipient_emails, email_address) do
    sender = Loanmanagementsystem.Email_configs.list_tbl_email_sender() |>List.first()
          new_email()
          |> from(sender.email)
          |> to("#{recipient_emails}")
          |> subject("Existing user(s)")
          |> assign(:email_address, email_address)
          |> put_html_layout({LoanmanagementsystemWeb.LayoutView, "email.html"})
          |> subject("Existing user(s)")
          |> render("existing_users.html")
          |> Mailer.deliver_later()
  end
