defmodule Loanmanagementsystem.Emails.Email do
  ...
  def reset_user_password_via_app(email, pwd, username, full_name) do
    new_email()
    |> from("johnmfula360@gmail.com")
    |> to("#{email}")
    |> put_html_layout({LoanmanagementsystemWeb.LayoutView, "email.html"})
    |> subject("GNC LMS Password Reset")
    |> text_body(
      """
      Dear #{full_name},
      Your Great North Credit Loan Management reset password is below.
      username: #{username}
      Password: #{pwd}
  ...
  def reset_user_password(email, password, customer_name, username)do
    new_email()
    |> from("johnmfula360@gmail.com")
    |> to("#{email}")
    |> put_html_layout({LoanmanagementsystemWeb.LayoutView, "email.html"})
    ...
    |> render("send_reset_password.html")
    |> Mailer.deliver_later()
  end
