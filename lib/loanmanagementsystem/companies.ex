defmodule Loanmanagementsystem.Companies do
  ...
  # Loanmanagementsystem.Companies.otc_currency_details_lookup(1)
  def otc_currency_details_lookup(id) do
    Country
    |> where([p], p.id == ^id)
    |> select([p], %{
      id: p.id,
  ...
  # Loanmanagementsystem.Companies.get_employer_by_code("A-5181")
  def get_employer_by_code(code) do
    Company
    |> where([c], c.employer_code == ^code)
    |> select([c], %{
      id: c.id,
      companyPhone: c.companyPhone,
      contactEmail: c.contactEmail,
