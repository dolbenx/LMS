defmodule Loanmanagementsystem.Services.InvoiceUploads do
  import Ecto.Query, warn: false
  alias Loanmanagementsystem.Repo
  alias Loanmanagementsystem.Loan.Loan_application_documents
  alias Loanmanagementsystem.Logs.UserLogs
# Loanmanagementsystem.Services.InvoiceUploads.
  def invoice_loan_upload(%{conn: conn, customer_id: customer_id, loan_id: loan_id}, params) do

    generate_path()
    |> case do
      {:ok, path} ->
        handle_documents(%{conn: conn, path: path, customer_id: customer_id, loan_id: loan_id}, _p = params)
  ...
  def handle_documents(%{conn: conn, path: path, customer_id: customer_id, loan_id: loan_id} = _p, params) do
    docx = params["file"]
    names = Map.keys(docx)
    ...
    random_string = Ecto.UUID.generate()
              ...
              Enum.map(0..len, fn n ->
                ...
                %{
                  ...
                  status: "ACTIVE",
                  customer_id: if is_integer(customer_id) do customer_id else  String.to_integer(customer_id) end,
                  path: dest,
                  doc_type: type,
                  fileName: name,
                  for_repayment: if is_nil(params["for_repayment"]) do false else true end,
