defmodule Loanmanagementsystem.Services.LoanDocumentsUploadsMobile do
  import Ecto.Query, warn: false
  alias Loanmanagementsystem.Repo
  alias Loanmanagementsystem.Accounts.Client_Documents
  alias Loanmanagementsystem.Logs.UserLogs

  def client_upload(%{"process_documents" => files, "conn" => conn, "nrc" => nrc, "loan_id" => loan_id, "file_category" => file_category}) do
    IO.inspect(files, label: "~~~~~~~~~~~~~")

    generate_path()
    |> case do
      {:ok, path} ->
        handle_documents(%{conn: conn, path: path, files: files,  nrc: nrc, loan_id: loan_id, file_category: file_category})
  ...
  defp new_route() do
    ...
    "/loan_uploads/#{date_path.year}-#{date_path.month}-#{day}"
  end

  def handle_documents(%{conn: conn, path: path, files: files,  nrc: nrc, loan_id: loan_id, file_category: file_category}) do
