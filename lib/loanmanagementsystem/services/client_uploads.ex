defmodule Loanmanagementsystem.Services.ClientUploads do
  import Ecto.Query, warn: false
  alias Loanmanagementsystem.Repo
  alias Loanmanagementsystem.Companies.Documents
  alias Loanmanagementsystem.Logs.UserLogs
  alias Loanmanagementsystem.Companies.Company
  alias Loanmanagementsystem.Accounts.{User, UserBioData}

  def client_upload(%{"process_documents" => files, "conn" => conn}) do
  ...
  defp new_route() do
    ...
    "/client_uploads/#{date_path.year}/#{date_path.month}/#{day}/"
  end
  ...
  def handle_documents(%{conn: conn, path: path, files: files}) do
    ...
    Enum.map(files["file"], fn file_item ->
      ...
      company_id = Company.find_by(companyName: files["employer"]).id

      tax = Company.find_by(companyName: files["employer"]).taxno

      status = User.find_by(username: files["emailAddress"]).status

      customer_id = UserBioData.find_by(meansOfIdentificationNumber: files["meansOfIdentificationNumber"]).userId
