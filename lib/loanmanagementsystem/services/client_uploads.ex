defmodule Loanmanagementsystem.Services.ClientUploads do
  ...
  def client_upload(%{"process_documents" => files, "conn" => conn, "company_id" => company_id, "individualId" => individualId, "nrc" => nrc}) do
    IO.inspect(files, label: "~~~~~~~~~~~~~")

    generate_path()
    |> case do
      {:ok, path} ->
        handle_documents(%{conn: conn, path: path, files: files, company_id: company_id, individualId: individualId, nrc: nrc})
  ...
  def handle_documents(%{conn: conn, path: path, files: files, company_id: company_id, individualId: individualId, nrc: nrc}) do
