defmodule Loanmanagementsystem.Services.EmployerUploads do
  ...
  def employer_upload(%{"process_documents" => files, "conn" => conn, "companyId" => companyId}) do
    IO.inspect(files, label: "~~~~~~~~~~~~~")

    generate_path()
    |> case do
      {:ok, path} ->
        handle_documents(%{conn: conn, path: path, files: files, companyId: companyId})
  ...
  def handle_documents(%{conn: conn, path: path, files: files, companyId: companyId}) do
    IO.inspect(files, label: "~~~~~~~~~000")
    # user = conn.assigns.user
    tax = Loanmanagementsystem.Companies.get_company!(companyId)
    ...
    {:ok, documents}
    |> handle_uploaded_files(conn, companyId)
  end

  def handle_uploaded_files(request, _conn, companyId) do
    case request do
      {:ok, documents} ->
        Ecto.Multi.new()
