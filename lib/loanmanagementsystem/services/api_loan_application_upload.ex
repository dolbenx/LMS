defmodule Loanmanagementsystem.Services.ApiApplicationUploads do
  import Ecto.Query, warn: false
  alias Loanmanagementsystem.Repo
  alias Loanmanagementsystem.Loan.Loan_application_documents
  alias Loanmanagementsystem.Logs.UserLogs

  def api_loan_application_upload(%{"conn" => conn, "customer_id" => customer_id, "loan_id" => loan_id}, params) do
  ...
  defp new_route() do
    ...
    "/employee_loan_application/#{date_path.year}/#{date_path.month}/#{day}/"
  end
  ...
  def handle_documents_new(%{conn: conn, path: path, customer_id: customer_id, loan_id: loan_id} = _p, params) do
    ...
    {:ok, documents}
    |> handle_uploaded_files(conn, customer_id)
  end

  def handle_uploaded_files(request, _conn, customer_id) do
    case request do
      {:ok, documents} ->
        Ecto.Multi.new()
