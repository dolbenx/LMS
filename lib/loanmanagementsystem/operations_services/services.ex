defmodule Loanmanagementsystem.OperationsServices do
  ...
  alias Loanmanagementsystem.Employment.{Employment_Details, Income_Details}
  ...
  alias Loanmanagementsystem.Companies.{Department, Employee_account}
  ...
  def get_individual_details_by_id(userID) do
    ...
    |> join(:left, [uS], pBD in Personal_Bank_Details, on: uS.id == pBD.userId)
    |> join(:left, [uS], iD in Income_Details, on: uS.id == iD.userId)
    |> join(:left, [uS], eA in Employee_account, on: uS.id == eA.employee_id)
    |> join(:left, [uS], eED in Employment_Details, on: uS.id == eED.userId)
    |> join(:left, [uS], cOM in Company, on: uS.company_id == cOM.id)


    |> where([uS, uB, uR, aD, kD], uS.id == ^userID)
    |> select([uS, uB, uR, aD, kD, pBD, iD, eA, eED, cOM], %{
      company_id: uS.company_id,
      classification_id: uS.classification_id,
      roleType: uR.roleType,
      status: uR.status,
      ...
      bank_name: fragment("concat(?, concat(' ', ?))", pBD.bankName, pBD.branchName),
      accountNumber: pBD.accountNumber,
      accountName: pBD.accountName,
      mobile_number: pBD.mobile_number,

      gross_pay: iD.gross_pay,
      total_deductions: iD.total_deductions,
      net_pay: iD.net_pay,
      pay_day: iD.pay_day,

      limit: eA.limit,

      employer_industry_type: eED.employer_industry_type,
      occupation: eED.occupation,
      date_of_joining: eED.date_of_joining,
      employee_number: eED.employee_number,



      companyName: cOM.registrationNumber,
      companyPhone: cOM.registrationNumber,
      contactEmail: cOM.registrationNumber,
      registrationNumber: cOM.registrationNumber,
