...
# Loanmanagementsystem.OperationsServices.get_client_business_details_by_user_id(24)
def get_client_individual_details(userId) do
  ...
  |> join(:left, [uS, uB, uR, eD, cO, aD], pbD in Personal_Bank_Details, on: uS.id == pbD.userId)
  |> join(:left, [uS, uB, uR, eD, cO, aD, pbD], ccD in Client_company_details, on: uS.id == ccD.createdByUserId)
  |> join(:left, [uS, uB, uR, eD, cO, aD, pbD, ccD], next_kin in Nextofkin, on: uS.id == next_kin.userID)

  |> where([uS, uB, uR, eD, cO, aD, ccD, next_kin], uS.id == ^userId)
  |> select([uS, uB, uR, eD, cO, aD, pbD, ccD, next_kin], %{
    company_id: uS.company_id,
    client_company_id: ccD.id,
    client_company_name: ccD.company_name,
    client_company_account_number: ccD.company_account_number,
    ...
    partners_full_names: uB.partners_full_names,
    partners_mobile_num: uB.partners_mobile_num,
    bank_id: uB.bank_id,
    bank_account_number: uB.bank_account_number,
    marital_status: uB.marital_status,
    nationality: uB.nationality,
    ...
    upload_bank_statement: pbD.upload_bank_statement,
    kin_id: next_kin.id,
    kin_first_name: next_kin.kin_first_name,
    kin_last_name: next_kin.kin_last_name,
    kin_other_name: next_kin.kin_other_name,
    kin_mobile_number: next_kin.kin_mobile_number,
    kin_gender: next_kin.kin_gender,
    kin_personal_email: next_kin.kin_personal_email,
    kin_relationship: next_kin.kin_relationship,
    kin_ID_number: next_kin.kin_ID_number,
    kin_ID_number: next_kin.kin_ID_number,
    applicant_signature_image: uB.applicant_signature_image,
  })
  |> distinct(true)
  |> limit(1)
  |> Repo.all()
...
def get_client_referes_by_user_id(userId) do
  Loanmanagementsystem.Accounts.Client_reference
  |> where([user_ref], user_ref.customer_id == ^userId)
  |> select([user_ref], %{
    id: user_ref.id,
    ref_full_name: user_ref.name,
    user_ref_contact_no: user_ref.contact_no,

  })
  |> Repo.all()
end

def get_client_market_details_by_user_id(userId) do
  Loanmanagementsystem.Loan.Loan_market_info
  |> where([mark], mark.customer_id == ^userId)
  |> select([mark], %{
    customer_id: mark.customer_id,
    reference_no: mark.reference_no,
    name_of_market: mark.name_of_market,
    location_of_market: mark.location_of_market,
    duration_at_market: mark.duration_at_market,
    type_of_business: mark.type_of_business,
    name_of_market_leader: mark.name_of_market_leader,
    mobile_of_market_leader: mark.mobile_of_market_leader,
    name_of_market_vice: mark.name_of_market_vice,
    mobile_of_market_vice: mark.mobile_of_market_vice,
    stand_number: mark.stand_number

  })
  |> Repo.all()
end

def get_client_employer_by_user_id(companyId) do
  Loanmanagementsystem.Companies.Company
  |> where([comp], comp.id == ^companyId)
  |> select([comp], %{
    id: comp.id,
    bank_id: comp.bank_id,
    companyName: comp.companyName,
    companyPhone: comp.companyPhone,
    registrationNumber: comp.registrationNumber,
    taxno: comp.taxno,
    contactEmail: comp.contactEmail,
    isEmployer: comp.isEmployer,
    isSme: comp.isSme,
    isOfftaker: comp.isOfftaker,
    createdByUserId: comp.createdByUserId,
    createdByUserRoleId: comp.createdByUserRoleId,
    status: comp.status,
    companyRegistrationDate: comp.companyRegistrationDate,
    companyAccountNumber: comp.companyAccountNumber,

  })
  |> Repo.all()
end

def get_client_business_details_by_user_id(userId) do
  Loanmanagementsystem.Employment.Employment_Details
  |> where([buz], buz.userId == ^userId)
  |> select([buz], %{
    employer: buz.employer,
    departmentId: buz.departmentId,
    occupation: buz.occupation,
    date_of_joining: buz.date_of_joining,
    employer_industry_type: buz.employer_industry_type,
    employment_type: buz.employment_type,
    employee_number: buz.employee_number,
    job_title: buz.job_title,
    hr_supervisor_name: buz.hr_supervisor_name,
    hr_supervisor_mobile_number: buz.hr_supervisor_mobile_number,
    hr_supervisor_email: buz.hr_supervisor_email,
    employer_office_building_name: buz.employer_office_building_name,
    employer_officer_street_name: buz.employer_officer_street_name,
    area: buz.area,
    town: buz.town,
    province: buz.province,
    userId: buz.userId,
    mobile_network_operator: buz.mobile_network_operator,
    registered_name_mobile_number: buz.registered_name_mobile_number,
    contract_start_date: buz.contract_start_date,
    contract_end_date: buz.contract_end_date
...
def get_loan_employememt_verification_docs(loan_id) do
  Client_Documents
  |> where([idvdoc], idvdoc.loan_id == ^loan_id and idvdoc.file_category == "EMPLOYMENT_VERIFICATION")
  |> select([idvdoc], %{
    id: idvdoc.id,
    docName: idvdoc.name,
    docPath: idvdoc.path,
...
def get_loan_legal_docs(loan_id) do
  Client_Documents
  |> where([idvdoc], idvdoc.loan_id == ^loan_id and idvdoc.file_category == "LEGAL_DOCUMENTS")
  |> select([idvdoc], %{
    id: idvdoc.id,
    docName: idvdoc.name,
    docPath: idvdoc.path,
...
def get_loan_docs(loan_id) do
  Client_Documents
  |> where([idvdoc], idvdoc.loan_id == ^loan_id and idvdoc.file_category == "LOAN_DOCUMENTS")
  |> select([idvdoc], %{
    id: idvdoc.id,
    docName: idvdoc.name,
    docPath: idvdoc.path,
...
def get_loan_crb_docs(loan_id) do
  Client_Documents
  |> where([idvdoc], idvdoc.loan_id == ^loan_id and idvdoc.file_category == "CRB_REPORT")
  |> select([idvdoc], %{
    id: idvdoc.id,
    docName: idvdoc.name,
    docPath: idvdoc.path,
...
def get_loan_property_docs(loan_id) do
  Client_Documents
  |> where([idvdoc], idvdoc.loan_id == ^loan_id and idvdoc.file_category == "PROPERTY_DOCUMENTS")
  |> select([idvdoc], %{
    id: idvdoc.id,
    docName: idvdoc.name,
    docPath: idvdoc.path,
