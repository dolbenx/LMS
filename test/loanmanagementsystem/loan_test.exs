defmodule Loanmanagementsystem.LoanTest do
  ...
  describe "tbl_payments_transaction" do
    alias Loanmanagementsystem.Loan.Payments_transaction

    @valid_attrs %{employee_id: 42, employee_number: "some employee_number", loan_id: 42, merchant_id: 42, payment_amount: 120.5, reference_no: "some reference_no"}
    @update_attrs %{employee_id: 43, employee_number: "some updated employee_number", loan_id: 43, merchant_id: 43, payment_amount: 456.7, reference_no: "some updated reference_no"}
    @invalid_attrs %{employee_id: nil, employee_number: nil, loan_id: nil, merchant_id: nil, payment_amount: nil, reference_no: nil}

    def payments_transaction_fixture(attrs \\ %{}) do
      {:ok, payments_transaction} =
        attrs
        |> Enum.into(@valid_attrs)
        |> Loan.create_payments_transaction()

      payments_transaction
    end

    test "list_tbl_payments_transaction/0 returns all tbl_payments_transaction" do
      payments_transaction = payments_transaction_fixture()
      assert Loan.list_tbl_payments_transaction() == [payments_transaction]
    end

    test "get_payments_transaction!/1 returns the payments_transaction with given id" do
      payments_transaction = payments_transaction_fixture()
      assert Loan.get_payments_transaction!(payments_transaction.id) == payments_transaction
    end

    test "create_payments_transaction/1 with valid data creates a payments_transaction" do
      assert {:ok, %Payments_transaction{} = payments_transaction} = Loan.create_payments_transaction(@valid_attrs)
      assert payments_transaction.employee_id == 42
      assert payments_transaction.employee_number == "some employee_number"
      assert payments_transaction.loan_id == 42
      assert payments_transaction.merchant_id == 42
      assert payments_transaction.payment_amount == 120.5
      assert payments_transaction.reference_no == "some reference_no"
    end

    test "create_payments_transaction/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Loan.create_payments_transaction(@invalid_attrs)
    end

    test "update_payments_transaction/2 with valid data updates the payments_transaction" do
      payments_transaction = payments_transaction_fixture()
      assert {:ok, %Payments_transaction{} = payments_transaction} = Loan.update_payments_transaction(payments_transaction, @update_attrs)
      assert payments_transaction.employee_id == 43
      assert payments_transaction.employee_number == "some updated employee_number"
      assert payments_transaction.loan_id == 43
      assert payments_transaction.merchant_id == 43
      assert payments_transaction.payment_amount == 456.7
      assert payments_transaction.reference_no == "some updated reference_no"
    end

    test "update_payments_transaction/2 with invalid data returns error changeset" do
      payments_transaction = payments_transaction_fixture()
      assert {:error, %Ecto.Changeset{}} = Loan.update_payments_transaction(payments_transaction, @invalid_attrs)
      assert payments_transaction == Loan.get_payments_transaction!(payments_transaction.id)
    end

    test "delete_payments_transaction/1 deletes the payments_transaction" do
      payments_transaction = payments_transaction_fixture()
      assert {:ok, %Payments_transaction{}} = Loan.delete_payments_transaction(payments_transaction)
      assert_raise Ecto.NoResultsError, fn -> Loan.get_payments_transaction!(payments_transaction.id) end
    end

    test "change_payments_transaction/1 returns a payments_transaction changeset" do
      payments_transaction = payments_transaction_fixture()
      assert %Ecto.Changeset{} = Loan.change_payments_transaction(payments_transaction)
    end
  end
end
