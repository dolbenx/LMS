defmodule Loanmanagementsystem.AccountsTest do
  ...
  describe "tbl_next_of_kin" do
    ...
    @valid_attrs %{applicant_nrc: "some applicant_nrc", kin_ID_number: "some kin_ID_number", kin_first_name: "some kin_first_name", kin_gender: "some kin_gender", kin_last_name: "some kin_last_name", kin_mobile_number: "some kin_mobile_number", kin_other_name: "some kin_other_name", kin_personal_email: "some kin_personal_email", kin_relationship: "some kin_relationship", kin_status: "some kin_status", userID: 42}
    @update_attrs %{applicant_nrc: "some updated applicant_nrc", kin_ID_number: "some updated kin_ID_number", kin_first_name: "some updated kin_first_name", kin_gender: "some updated kin_gender", kin_last_name: "some updated kin_last_name", kin_mobile_number: "some updated kin_mobile_number", kin_other_name: "some updated kin_other_name", kin_personal_email: "some updated kin_personal_email", kin_relationship: "some updated kin_relationship", kin_status: "some updated kin_status", userID: 43}
    @invalid_attrs %{applicant_nrc: nil, kin_ID_number: nil, kin_first_name: nil, kin_gender: nil, kin_last_name: nil, kin_mobile_number: nil, kin_other_name: nil, kin_personal_email: nil, kin_relationship: nil, kin_status: nil, userID: nil}

    def nextofkin_fixture(attrs \\ %{}) do
      {:ok, nextofkin} =
        attrs
        |> Enum.into(@valid_attrs)
        |> Accounts.create_nextofkin()
    ...
    test "list_tbl_next_of_kin/0 returns all tbl_next_of_kin" do
      ...
      assert Accounts.list_tbl_next_of_kin() == [nextofkin]
    end

    test "get_nextofkin!/1 returns the nextofkin with given id" do
      nextofkin = nextofkin_fixture()
      assert Accounts.get_nextofkin!(nextofkin.id) == nextofkin
    end

    test "create_nextofkin/1 with valid data creates a nextofkin" do
      assert {:ok, %Nextofkin{} = nextofkin} = Accounts.create_nextofkin(@valid_attrs)
      assert nextofkin.applicant_nrc == "some applicant_nrc"
      assert nextofkin.kin_ID_number == "some kin_ID_number"
      assert nextofkin.kin_first_name == "some kin_first_name"
      assert nextofkin.kin_gender == "some kin_gender"
      assert nextofkin.kin_last_name == "some kin_last_name"
      assert nextofkin.kin_mobile_number == "some kin_mobile_number"
      assert nextofkin.kin_other_name == "some kin_other_name"
      assert nextofkin.kin_personal_email == "some kin_personal_email"
      assert nextofkin.kin_relationship == "some kin_relationship"
      assert nextofkin.kin_status == "some kin_status"
      assert nextofkin.userID == 42
    end

    test "create_nextofkin/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Accounts.create_nextofkin(@invalid_attrs)
    end

    test "update_nextofkin/2 with valid data updates the nextofkin" do
      nextofkin = nextofkin_fixture()
      assert {:ok, %Nextofkin{} = nextofkin} = Accounts.update_nextofkin(nextofkin, @update_attrs)
      assert nextofkin.applicant_nrc == "some updated applicant_nrc"
      assert nextofkin.kin_ID_number == "some updated kin_ID_number"
      assert nextofkin.kin_first_name == "some updated kin_first_name"
      assert nextofkin.kin_gender == "some updated kin_gender"
      assert nextofkin.kin_last_name == "some updated kin_last_name"
      assert nextofkin.kin_mobile_number == "some updated kin_mobile_number"
      assert nextofkin.kin_other_name == "some updated kin_other_name"
      assert nextofkin.kin_personal_email == "some updated kin_personal_email"
      assert nextofkin.kin_relationship == "some updated kin_relationship"
      assert nextofkin.kin_status == "some updated kin_status"
      assert nextofkin.userID == 43
    end

    test "update_nextofkin/2 with invalid data returns error changeset" do
      nextofkin = nextofkin_fixture()
      assert {:error, %Ecto.Changeset{}} = Accounts.update_nextofkin(nextofkin, @invalid_attrs)
      assert nextofkin == Accounts.get_nextofkin!(nextofkin.id)
    end

    test "delete_nextofkin/1 deletes the nextofkin" do
      nextofkin = nextofkin_fixture()
      assert {:ok, %Nextofkin{}} = Accounts.delete_nextofkin(nextofkin)
      assert_raise Ecto.NoResultsError, fn -> Accounts.get_nextofkin!(nextofkin.id) end
    end

    test "change_nextofkin/1 returns a nextofkin changeset" do
      nextofkin = nextofkin_fixture()
      assert %Ecto.Changeset{} = Accounts.change_nextofkin(nextofkin)
    end
  end

  describe "tbl_client_reference" do
    alias Loanmanagementsystem.Accounts.Client_reference

    @valid_attrs %{contact_no: "some contact_no", customer_id: 42, name: "some name", status: "some status"}
    @update_attrs %{contact_no: "some updated contact_no", customer_id: 43, name: "some updated name", status: "some updated status"}
    @invalid_attrs %{contact_no: nil, customer_id: nil, name: nil, status: nil}

    def client_reference_fixture(attrs \\ %{}) do
      {:ok, client_reference} =
        attrs
        |> Enum.into(@valid_attrs)
        |> Accounts.create_client_reference()

      client_reference
    end

    test "list_tbl_client_reference/0 returns all tbl_client_reference" do
      client_reference = client_reference_fixture()
      assert Accounts.list_tbl_client_reference() == [client_reference]
    end

    test "get_client_reference!/1 returns the client_reference with given id" do
      client_reference = client_reference_fixture()
      assert Accounts.get_client_reference!(client_reference.id) == client_reference
    end

    test "create_client_reference/1 with valid data creates a client_reference" do
      assert {:ok, %Client_reference{} = client_reference} = Accounts.create_client_reference(@valid_attrs)
      assert client_reference.contact_no == "some contact_no"
      assert client_reference.customer_id == 42
      assert client_reference.name == "some name"
      assert client_reference.status == "some status"
    end

    test "create_client_reference/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Accounts.create_client_reference(@invalid_attrs)
    end

    test "update_client_reference/2 with valid data updates the client_reference" do
      client_reference = client_reference_fixture()
      assert {:ok, %Client_reference{} = client_reference} = Accounts.update_client_reference(client_reference, @update_attrs)
      assert client_reference.contact_no == "some updated contact_no"
      assert client_reference.customer_id == 43
      assert client_reference.name == "some updated name"
      assert client_reference.status == "some updated status"
    end

    test "update_client_reference/2 with invalid data returns error changeset" do
      client_reference = client_reference_fixture()
      assert {:error, %Ecto.Changeset{}} = Accounts.update_client_reference(client_reference, @invalid_attrs)
      assert client_reference == Accounts.get_client_reference!(client_reference.id)
    end

    test "delete_client_reference/1 deletes the client_reference" do
      client_reference = client_reference_fixture()
      assert {:ok, %Client_reference{}} = Accounts.delete_client_reference(client_reference)
      assert_raise Ecto.NoResultsError, fn -> Accounts.get_client_reference!(client_reference.id) end
    end

    test "change_client_reference/1 returns a client_reference changeset" do
      client_reference = client_reference_fixture()
      assert %Ecto.Changeset{} = Accounts.change_client_reference(client_reference)
    end
  end
end
