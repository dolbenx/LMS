defmodule Loanmanagementsystem.CompaniesTest do
  ...
  describe "tbl_employee_account" do
    alias Loanmanagementsystem.Companies.Employee_account

    @valid_attrs %{balance: 120.5, employee_id: 42, employee_number: "some employee_number", limit: 120.5, status: "some status"}
    @update_attrs %{balance: 456.7, employee_id: 43, employee_number: "some updated employee_number", limit: 456.7, status: "some updated status"}
    @invalid_attrs %{balance: nil, employee_id: nil, employee_number: nil, limit: nil, status: nil}

    def employee_account_fixture(attrs \\ %{}) do
      {:ok, employee_account} =
        attrs
        |> Enum.into(@valid_attrs)
        |> Companies.create_employee_account()

      employee_account
    end

    test "list_tbl_employee_account/0 returns all tbl_employee_account" do
      employee_account = employee_account_fixture()
      assert Companies.list_tbl_employee_account() == [employee_account]
    end

    test "get_employee_account!/1 returns the employee_account with given id" do
      employee_account = employee_account_fixture()
      assert Companies.get_employee_account!(employee_account.id) == employee_account
    end

    test "create_employee_account/1 with valid data creates a employee_account" do
      assert {:ok, %Employee_account{} = employee_account} = Companies.create_employee_account(@valid_attrs)
      assert employee_account.balance == 120.5
      assert employee_account.employee_id == 42
      assert employee_account.employee_number == "some employee_number"
      assert employee_account.limit == 120.5
      assert employee_account.status == "some status"
    end

    test "create_employee_account/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Companies.create_employee_account(@invalid_attrs)
    end

    test "update_employee_account/2 with valid data updates the employee_account" do
      employee_account = employee_account_fixture()
      assert {:ok, %Employee_account{} = employee_account} = Companies.update_employee_account(employee_account, @update_attrs)
      assert employee_account.balance == 456.7
      assert employee_account.employee_id == 43
      assert employee_account.employee_number == "some updated employee_number"
      assert employee_account.limit == 456.7
      assert employee_account.status == "some updated status"
    end

    test "update_employee_account/2 with invalid data returns error changeset" do
      employee_account = employee_account_fixture()
      assert {:error, %Ecto.Changeset{}} = Companies.update_employee_account(employee_account, @invalid_attrs)
      assert employee_account == Companies.get_employee_account!(employee_account.id)
    end

    test "delete_employee_account/1 deletes the employee_account" do
      employee_account = employee_account_fixture()
      assert {:ok, %Employee_account{}} = Companies.delete_employee_account(employee_account)
      assert_raise Ecto.NoResultsError, fn -> Companies.get_employee_account!(employee_account.id) end
    end

    test "change_employee_account/1 returns a employee_account changeset" do
      employee_account = employee_account_fixture()
      assert %Ecto.Changeset{} = Companies.change_employee_account(employee_account)
    end
  end
end
