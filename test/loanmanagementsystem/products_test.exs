defmodule Loanmanagementsystem.ProductsTest do
  ...
  describe "product_id" do
    ...
    @valid_attrs %{charge_id: "some charge_id", status: "some status"}
    @update_attrs %{charge_id: "some updated charge_id", status: "some updated status"}
    @invalid_attrs %{charge_id: nil, status: nil}

    def product_charges_fixture(attrs \\ %{}) do
      {:ok, product_charges} =
        attrs
        |> Enum.into(@valid_attrs)
        |> Products.create_product_charges()

      product_charges
    end

    test "list_product_id/0 returns all product_id" do
      product_charges = product_charges_fixture()
      assert Products.list_product_id() == [product_charges]
    end

    test "get_product_charges!/1 returns the product_charges with given id" do
      product_charges = product_charges_fixture()
      assert Products.get_product_charges!(product_charges.id) == product_charges
    end

    test "create_product_charges/1 with valid data creates a product_charges" do
      assert {:ok, %Product_charges{} = product_charges} = Products.create_product_charges(@valid_attrs)
      assert product_charges.charge_id == "some charge_id"
      assert product_charges.status == "some status"
    end

    test "create_product_charges/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Products.create_product_charges(@invalid_attrs)
    end

    test "update_product_charges/2 with valid data updates the product_charges" do
      product_charges = product_charges_fixture()
      assert {:ok, %Product_charges{} = product_charges} = Products.update_product_charges(product_charges, @update_attrs)
      assert product_charges.charge_id == "some updated charge_id"
    ...
    test "update_product_charges/2 with invalid data returns error changeset" do
      product_charges = product_charges_fixture()
      assert {:error, %Ecto.Changeset{}} = Products.update_product_charges(product_charges, @invalid_attrs)
      assert product_charges == Products.get_product_charges!(product_charges.id)
    end

    test "delete_product_charges/1 deletes the product_charges" do
      product_charges = product_charges_fixture()
      assert {:ok, %Product_charges{}} = Products.delete_product_charges(product_charges)
      assert_raise Ecto.NoResultsError, fn -> Products.get_product_charges!(product_charges.id) end
    end

    test "change_product_charges/1 returns a product_charges changeset" do
      product_charges = product_charges_fixture()
      assert %Ecto.Changeset{} = Products.change_product_charges(product_charges)
    end
  end
end
