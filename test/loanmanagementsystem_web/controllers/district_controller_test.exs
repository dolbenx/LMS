defmodule LoanmanagementsystemWeb.DistrictControllerTest do
  ...
  def fixture(:district) do
    {:ok, district} = Maintenance.create_district(@create_attrs)
    district
  end

  describe "index" do
    test "lists all tbl_district", %{conn: conn} do
      conn = get(conn, Routes.district_path(conn, :index))
      assert html_response(conn, 200) =~ "Listing Tbl district"
    end
  end
